...
%ELSE %DO;	
	...
	%END;
		...
		DATA BaseFmapFmap;
		SET tempAccuFmap&Nlag(WHERE=(PaymentMonth <= &lastDate));
		RUN;

	PROC SQL NOPRINT;
		CREATE TABLE FMAP.FmapMos&category._&service AS
		SELECT a.*, b.*
		FROM BaseFmapFmap AS a, FMAP.PctExpenditure(WHERE=(ExpenditureCategoryMcFfs="&category" AND
...
%if	(&category=1222x and &service=310) or
	(&category=1222X and &service=333) or
	(&category=1221X and &service=333) 
...
%if (&category=1222x and &service=211) or
	(&category=1222 and &service=221) or
	(&category=1222 and &service=290) or
	(&category=1222 and &service=375) or	
	(&category=1221x and &service=671) or
	(&category=1221x and &service=310) or
...
%then %do;
	data FMAP.FmapMos&category._&service;
		set FMAP.FmapMos&category._&service;
		LagValue = Lag(FmapMOS);
		lagvalue2= lag2(FmapMOS);
		lagvalue3 = lag3(FmapMOS);
			...
			if PaymentMonth = '01DEC18'd then FmapMOS = LagValue;
            /*zxg:newly added */
/*            if PaymentMonth = '01Aug19'd then FmapMOS = lagvalue;	*/
/*			if PaymentMonth = '01Sep19'd then FmapMOS = lagvalue2;	*/
/*            if PaymentMonth = '01Oct19'd then FmapMOS = lagvalue2;	*/
/*			if PaymentMonth = '01NOV19'd then FmapMOS = lagvalue3;				*/
/*			if PaymentMonth = '01DEC19'd then FmapMOS = lagvalue3;*/
...
/*zxg: newly added, to deal with the FAMP declient to much !!!*/
%if (&category=1861 and &service=551) %then %do;
   %put NOTE: I am here ++++++++++++++++++++++++++++; 
   data FMAP.FmapMos&category._&service;
		set FMAP.FmapMos&category._&service;
		LagValue = Lag(FmapMOS);
		LagValue2 = Lag2(FmapMOS);
		if PaymentMonth = '01Jul19'd then FmapMOS = LagValue;
        if PaymentMonth = '01Aug19'd then FmapMOS = LagValue;
		if PaymentMonth = '01Sep19'd then FmapMOS = LagValue;
		if PaymentMonth = '01OCT19'd then FmapMOS = LagValue2;
		

		

		if PaymentMonth = '01Jul20'd then FmapMOS = LagValue;
        if PaymentMonth = '01Aug20'd then FmapMOS = LagValue;
		if PaymentMonth = '01Sep20'd then FmapMOS = LagValue;
		if PaymentMonth = '01Oct20'd then FmapMOS = LagValue2;
		
     drop LagValue LagValue2;	
	run;
  %end;
...
Data Fmap.FmapRatio&category._&service; Set Fmap.Dumster; 
	ForecastVersionID="A&pDataCycle";
