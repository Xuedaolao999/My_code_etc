...
/*--------------------------- zxg: modified on 1/18/2023 ------------------------*/
%LET dsid=%SYSFUNC(OPEN(FMAP.ForecastPTmodelsCurrent(WHERE=(UPCASE(test)='X'))));
...
DATA FMAP.FmapRatio&category._&service;	SET FMAP.FmapRatio&category._&service;
...
/*####################################################################################################*/
	...
	PROC SQL NOPRINT;
		CREATE TABLE FMAP.FmapRatio&category._&service as
		SELECT a.*
				/*b.FedShareValue AS CurrentFedShare,*/
				/*COALESCE(b.FedShareValue/a.FmapMOS, 1) AS CurrentFmapRatio*/
       ...
       /*ZXG: create table MEG and Svc here, removing the below proc sort procedures */
		create table MEG as 
		select strip(ForMegName) as ForMegName length=48, ForMeg
		from MainDM.Dim_for_MEG
		order by ForMeg;

		create table Svc as 
		select strip(ForSvcName) as ForSvcName length = 56,ForSvc
		from MainDM.Dim_for_Svc
		order by ForSvc;
   %put NOTE: i am here to remove the sort proc;
...
DATA FMAP.Result_Fedshare;  SET FMAP.Result_Fedshare;
		IF ForecastVersionID = "A&pDataCycle" AND
			ForecastMeg = "&category" AND
			ForecastSvc = "&service" THEN Delete;
	RUN;

	DATA FMAP.PredResult_Fedshare;  SET FMAP.PredResult_Fedshare;
		IF ForecastVersionID = "A&pDataCycle" AND
			ForecastMeg = "&category" AND
			ForecastSvc = "&service" THEN Delete;
	RUN;
...
/* ZXG: cut the FedShareValue in the prediction period */
data Fmap.FmapRatio&Category._&Service;
  set Fmap.FmapRatio&Category._&Service;
  if MOS > &LastDateOfAcutalData THEN FedShareValue = .;
run; 
	...
	*ODS PDF FILE="&pDMosGraphs.&ModelType._Fmap_&category._&service.__%unquote(%sysfunc(datetime(),mydtfmt.)).pdf" STYLE=SASWEB;
	...
	Proc GPlot data=FmapRatio&category._&service._Graph;
		axis1 label=(angle=90 "FMAP") 
          %if &category= 1350 and &service = 453 %then order=(0 to 1.5 by 0.1 );;/*zxg: change the scale modified on 8/11/2021*/; 
		axis2 label=("Month of Service") minor=none;
		PLOT 	
...
/*SOF graphs : ZXG: REMOVE the macro sof */
/*%macro sof;*/
