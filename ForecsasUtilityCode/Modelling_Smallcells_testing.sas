...
%if %sysfunc(exist(PmTrend.cell_25_missing)) %then %do;
   ...
   %end;
%if %sysfunc(exist(PmTrend.cell_25_missing_prd)) %then %do;
	  %put NOTE: the table PmTrend.cell_25_missing_prd exists!;
	%end;
	%else %do;
proc sql;
     create table PmTrend.cell_25_missing_prd 
         ( MEG char(4) not null,
           SVC char(3) not null,
		   MOS num format=MONYY7.,
           Actual num,
           Pred num 
          );
quit; 
	...
	%CPESpecialSmallCell(&category, &Service, A);
      ...
      %END;
		 ...
		 %if &n_obs = 0 %then %do;
		   ...
		   proc sql noprint;
		     select count(*) into:_nrow2
			   from PmTrend.cell_25_missing_prd
			   where meg = "&category" and svc = "&service";
             %if &_nrow2 = 0 %then %do;
               insert into PmTrend.cell_25_missing_prd(MEG, SVC, MOS,Actual)
                select "&category" as meg,"&service" as svc, date format=MONYY7.,percap
                from PmTrend.UcmInputData_&category._&service;
  
                update PmTrend.cell_25_missing_prd
                  set pred = (select sum(perCap)
                             from PmTrend.UcmInputData_&category._&service 
							 %if &FirstAcutalDataDelta = 0 %then %do;
							  where  perCap^=. 
							 %end;
							 %else %do;
                             where date>= %SYSFUNC(INTNX(MONTH, &lastDateofacutalData, -&FirstAcutalDataDelta)) and perCap^=.
							 %end;
                           )/(intck('month',%SYSFUNC(INTNX(MONTH, &lastDateofacutalData, -&ScCutOff)),&Pred_last_date)+1)
	            where Meg="&category" and svc="&service" and mos >=%SYSFUNC(INTNX(MONTH, &lastDateofacutalData, -&ScCutOff));
             %end;
             quit; 
		   ods pdf close;
		   %goto done;
