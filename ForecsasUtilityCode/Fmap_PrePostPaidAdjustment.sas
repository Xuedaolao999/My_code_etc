...
%let cell_list = prepay post pay FMAP FC2517.xlsx;
...
%macro loop_cell;
  %let dsid = %sysfunc(open(cell_list));
   ...
   %do %while(%sysfunc(fetch(&dsid)) eq 0);
     %PUT NOTE:  I am in cell >>>>>>(&ForMeg, &ForSvc, &PayOffset)<<<<< ;
	 %if %eval(&PayOffset<0) %then %do;
        %put i am negative;
		%backward_cell(ForecastMeg = &ForMeg,ForecastSvc = &ForSvc,PayOffset = &PayOffset);
		%end;
	 %else %do;
        %put i am positive;
		%forward_cell( ForecastMeg = &ForMeg,ForecastSvc = &ForSvc,PayOffset = &PayOffset);
     %end;
   %end;
