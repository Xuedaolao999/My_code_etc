/*
~ METHOD_NAME                      - Modelling_Smallcells
~ vCLASS_ID                        - 8 
~ EXEC_CALL                        - 0
~ INPUT_IMPLEMENTATION_FORM        - NA 
~ METHOD_RETURN_VALUE              - NA 
~ RETURN_IMPLEMENTATION_FORM       - NA 
~ METHOD_LABEL                     - Modelling_Smallcells
~ METHOD_DESCRIPTION               - Modelling Small cells
~ PARAMETER_NAME                   - test
~ PARAMETER_VALUE                  - NULL 
~ PROGRAMMER                       - Xingguo Zhang
~ DATE_CREATED                     - 2016/11/2
*/
dm 'log' clear;
...
%LET dsidm=%SYSFUNC(OPEN(pmtrend.smallcells(WHERE=(upcase(selected)="X"))));
	...
	%DO %WHILE(%SYSFUNC(FETCH(&dsidm)) EQ 0);
			...
			%LET UCMauto=%SYSFUNC(GETVARN(&dsidm, %SYSFUNC(VARNUM(&dsidm, UCMauto))));
...
/* zxg: modified on 3/13/2023                                          */
          %ucm_proc(cat=&category,svc=&service,ld=&lead);
           ...
           %if &UCMauto = 1 %then %do; %UCMauto %end; /*Run Nhan's auto UCM code*/
...
/* zxg: this is important to change the value */
	...
	Data PmTrend.UcmSts_&category._&service; 
	  ...
	  if date<=&plot_local_dt and PriorPerCap=0 then PriorPerCap = .; /*zxg:modify the plot on 1/10/2024  */
...
/*  Data PmTrend.UcmSts_&category._&service; */
/*    set Data PmTrend.UcmSts_&category._&service; */
/*	if date<="&plot_local_dt" and PriorPerCap=0 then PriorPerCap = .;*/
/*  run; */
